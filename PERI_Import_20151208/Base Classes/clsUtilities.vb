            ...
            Try
                ...
                If oProcesses.Length <> 0 Then
                    For i As Integer = 0 To oProcesses.Length - 1
                        ...
                        oDialogBox.Filter = " Excel | *.xls;*.xlsx"
...
#End Region
    ...
    Public Function GetExcelData(ByVal oForm As SAPbouiCOM.Form, ByVal strID As String) As Boolean
        ...
        oRecordSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        Try
            _oDt.TableName = "PEIMPORT"

            _oDt.Columns.Add("InvoiceNo", GetType(String)).Caption = "InvoiceNo"
            _oDt.Columns.Add("CardCode", GetType(String)).Caption = "CardCode"
            _oDt.Columns.Add("CardName", GetType(String)).Caption = "CardName"
            _oDt.Columns.Add("DocDate", GetType(String)).Caption = "DocDate"
            _oDt.Columns.Add("DueDate", GetType(String)).Caption = "DueDate"
            _oDt.Columns.Add("Project", GetType(String)).Caption = "Project"
            _oDt.Columns.Add("LineTotal", GetType(Double)).Caption = "LineTotle"
            _oDt.Columns.Add("DocTotal", GetType(Double)).Caption = "DocTotal"
            _oDt.Columns.Add("Line", GetType(Double)).Caption = "Line"
            _oDt.Columns.Add("RevAcct", GetType(Double)).Caption = "Revenue Account"
            _oDt.Columns.Add("Currency", GetType(String)).Caption = "Currency"
            _oDt.Columns.Add("DocType", GetType(String)).Caption = "Document Type"

            Dim strPath As String = CType(oForm.Items.Item(strID).Specific, SAPbouiCOM.StaticText).Caption
            If strPath.Length > 0 Then
                If (oApplication.Excel.getExcelTemplate(strPath, _oDt)) Then
                    Dim strDtXML As String = getXMLstring(_oDt)
                    oRecordSet.DoQuery("Exec [Insert_PERIImport] '" + strDtXML + "'")
                    _retVal = True
                End If
            End If
    ...
    Public Function Import_PERI_InvoiceData(ByVal oForm As SAPbouiCOM.Form) As Boolean
        Dim _retVal As Boolean = False
        Dim oSInvoice As SAPbobsCOM.Documents = Nothing
        Dim oSCreditNote As SAPbobsCOM.Documents = Nothing

        Try
            Dim intStatus As Integer

            Dim intRow As Integer = 0
            Dim oInvoiceSet As SAPbobsCOM.Recordset
            Dim oDetailSet As SAPbobsCOM.Recordset
            Dim oGrid As SAPbouiCOM.Grid = oForm.Items.Item("8").Specific

            oInvoiceSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oDetailSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oSInvoice = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oInvoices)
            oSCreditNote = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oCreditNotes)
            oInvoiceSet.DoQuery("Select * from [@Z_PERI_SERIES]")
            Dim blnManual As Boolean = True
            If oInvoiceSet.RecordCount > 0 Then
                If oInvoiceSet.Fields.Item("U_Z_Series").Value <> "M" Then
                    blnManual = False
                End If
            End If
            ...
            'Sale Invoice
            Dim strQuery As String = "Select Distinct InvoiceNo From Z_PEIM Where Line > 0 And DocType Not IN ('80','82')"
            oInvoiceSet.DoQuery(strQuery)
            oApplication.Utilities.Message("Processing AR Invoice creations....", SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
            If Not oInvoiceSet.EoF Then
                While Not oInvoiceSet.EoF
                    Dim strInvoice As String = oInvoiceSet.Fields.Item(0).Value
                    ...
                    strQuery += " And T0.InvoiceNo = '" + strInvoice + "' And DocType Not IN ('80','82')"
                    oDetailSet.DoQuery(strQuery)

                    If Not oDetailSet.EoF Then
                        oSInvoice = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oInvoices)
                        Dim intLine As Integer = 0
                        If blnManual = True Then
                            oSInvoice.HandWritten = SAPbobsCOM.BoYesNoEnum.tYES
                            oSInvoice.DocNum = oDetailSet.Fields.Item("InvoiceNo").Value
                            oSInvoice.NumAtCard = oDetailSet.Fields.Item("InvoiceNo").Value
                        Else
                            oSInvoice.HandWritten = SAPbobsCOM.BoYesNoEnum.tNO
                            oSInvoice.NumAtCard = oDetailSet.Fields.Item("InvoiceNo").Value
                        End If
                       
                        oSInvoice.DocType = SAPbobsCOM.BoDocumentTypes.dDocument_Service
                        oSInvoice.CardCode = oDetailSet.Fields.Item("CardCode").Value
                        oSInvoice.DocDate = oDetailSet.Fields.Item("DocDate").Value
                        oSInvoice.DocDueDate = oDetailSet.Fields.Item("DueDate").Value
                        oSInvoice.Reference1 = oDetailSet.Fields.Item("Project").Value
                        oSInvoice.DocCurrency = oDetailSet.Fields.Item("Currency").Value

                        While Not oDetailSet.EoF
                            Dim strCurrency As String = oDetailSet.Fields.Item("Currency").Value
                            If intLine > 0 Then
                                oSInvoice.Lines.Add()
                                oSInvoice.Lines.SetCurrentLine(intLine)
                            End If
                            oSInvoice.Lines.TaxCode = "XO"
                            oSInvoice.Lines.LocationCode = 1
                            oSInvoice.Lines.AccountCode = oDetailSet.Fields.Item("RevAcct").Value
                            oSInvoice.Lines.Currency = oDetailSet.Fields.Item("Currency").Value
                            oSInvoice.Lines.LineTotal = oDetailSet.Fields.Item("LineTotal").Value
                            intLine = intLine + 1
                            oDetailSet.MoveNext()
                        End While

                        intStatus = oSInvoice.Add()
                        If intStatus <> 0 Then
                            Throw New Exception(oApplication.Company.GetLastErrorDescription())
                        Else
                            _retVal = False
                    ...
                    oInvoiceSet.MoveNext()
                End While
            End If
            oApplication.Utilities.Message("Processing AR Credit Note creations....", SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
            'Credit Note
            strQuery = "Select Distinct InvoiceNo From Z_PEIM Where Line > 0 And DocType IN ('80','82')"
            oInvoiceSet.DoQuery(strQuery)
            If Not oInvoiceSet.EoF Then
                While Not oInvoiceSet.EoF
                    Dim strInvoice As String = oInvoiceSet.Fields.Item(0).Value
                    ...
                    strQuery += " JOIN OACT T2 On T0.RevAcct =  T2.AcctCode "
                    strQuery += " And T0.Line > 0 "
                    strQuery += " And T0.InvoiceNo = '" + strInvoice + "' And DocType IN ('80','82')"
                    oDetailSet.DoQuery(strQuery)
                    ...
                    If Not oDetailSet.EoF Then
                        ...
                        'oSCreditNote.HandWritten = SAPbobsCOM.BoYesNoEnum.tYES
                        'oSCreditNote.DocNum = oDetailSet.Fields.Item("InvoiceNo").Value
                        If blnManual = True Then
                            oSCreditNote.HandWritten = SAPbobsCOM.BoYesNoEnum.tYES
                            oSCreditNote.DocNum = oDetailSet.Fields.Item("InvoiceNo").Value
                            oSCreditNote.NumAtCard = oDetailSet.Fields.Item("InvoiceNo").Value
                        Else
                            oSCreditNote.HandWritten = SAPbobsCOM.BoYesNoEnum.tNO
                            oSCreditNote.NumAtCard = oDetailSet.Fields.Item("InvoiceNo").Value
                        End If
                        oSCreditNote.DocType = SAPbobsCOM.BoDocumentTypes.dDocument_Service
                        oSCreditNote.CardCode = oDetailSet.Fields.Item("CardCode").Value
                        oSCreditNote.DocDate = oDetailSet.Fields.Item("DocDate").Value
                        oSCreditNote.DocDueDate = oDetailSet.Fields.Item("DueDate").Value
                        oSCreditNote.Reference1 = oDetailSet.Fields.Item("Project").Value
                        oSCreditNote.DocCurrency = oDetailSet.Fields.Item("Currency").Value

                        While Not oDetailSet.EoF
                            Dim strCurrency As String = oDetailSet.Fields.Item("Currency").Value
                            If intLine > 0 Then
                                oSCreditNote.Lines.Add()
                                oSCreditNote.Lines.SetCurrentLine(intLine)
                            End If
                            oSCreditNote.Lines.TaxCode = "XO"
                            oSCreditNote.Lines.LocationCode = 1
                            oSCreditNote.Lines.AccountCode = oDetailSet.Fields.Item("RevAcct").Value
                            oSCreditNote.Lines.Currency = oDetailSet.Fields.Item("Currency").Value

                            If oDetailSet.Fields.Item("LineTotal").Value < 0 Then
                                oSCreditNote.Lines.LineTotal = oDetailSet.Fields.Item("LineTotal").Value * -1
                            Else
                                oSCreditNote.Lines.LineTotal = oDetailSet.Fields.Item("LineTotal").Value
                            End If
            ...
            If oApplication.Company.InTransaction Then
                oApplication.Company.EndTransaction(SAPbobsCOM.BoWfTransOpt.wf_Commit)
                oApplication.Utilities.Message("Operation completed successfully....", SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                _retVal = True
            End If
        ...
        Catch ex As Exception
            ...
            Throw ex
        Finally
            System.Runtime.InteropServices.Marshal.ReleaseComObject(oSInvoice)
            System.Runtime.InteropServices.Marshal.ReleaseComObject(oSCreditNote)
        End Try
    End Function
    ...
    '            oSInvoice.HandWritten = SAPbobsCOM.BoYesNoEnum.tYES
    '            oSInvoice.DocNum = oGrid.DataTable.GetValue("InvoiceNo", index)
    '            oSInvoice.DocType = SAPbobsCOM.BoDocumentTypes.dDocument_Service
    '            oSInvoice.CardCode = oGrid.DataTable.GetValue("CardCode", index)
    '            'oSInvoice.CardName = oGrid.DataTable.GetValue("CardName", index)

    '            oSInvoice.DocDate = oGrid.DataTable.GetValue("DocDate", index)
    '            oSInvoice.DocDueDate = oGrid.DataTable.GetValue("DueDate", index)
    '            oSInvoice.Reference1 = oGrid.DataTable.GetValue("Project", index)

    '            oSInvoice.Lines.TaxCode = "XO"
    '            oSInvoice.Lines.LocationCode = 1
    '            oSInvoice.Lines.AccountCode = oGrid.DataTable.GetValue("RevAcct", index)
    '            oSInvoice.Lines.LineTotal = oGrid.DataTable.GetValue("LineTotal", index)
